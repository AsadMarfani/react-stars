...
class ReactStars extends Component {
  ...
  mouseOver(event) {
    ...
    const configState = Object.assign({}, this.state.config, { showStatus: true });
    this.setState({
  ...
  mouseLeave() {
    ...
    const configState = Object.assign({}, this.state.config, { showStatus: false });
    this.setState({
      stars: this.getStars(),
      config: configState
    });
  ...
  renderStars() {
    ...
    return stars.map((star, i) => {
      ...
      const status =
        selectedStars.length === i + 1 ? this.props.status[i + 1] : "";

      return (
        <span key={i}>
          <span className={starClass} style={style}>
            <span style={statusStyle}>{showStatus && status}</span>
            <span
              data-index={i}
              data-forhalf={char}
